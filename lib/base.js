...
const setAccount = function (host, port, user, password, database, eqInt) { 
    return new Promsie((resolve, reject) => {
        ...
        try {
            ...
            reject(new Error(`ERR[mysqlCntlError]: ${e}`));
        }
    })
}
...
const query = function (q_comm) {
    return new Promise((resolve, reject) => {
        if (connMysql == "") reject(new Error(`ERR[mysqlCntlError]: Cannot perform query - setAccount is not set`));
        conn_mysql.query(q_comm, (err, rows, fields) => {
        if (err) {
                reject(errorHandler(`Query ERR`, err))
            } else {
                resolve(rows);
            }
        })
